...
namespace WindowsSocketCommon
{
    ...
    // of time represented by 'class_ts_bt' has passed.
    int doBlockingTimeout(SOCKET socket_fd, short events, INT& class_ts_bt);

    // Associates the given socket with a given port number ("names" the
    ...
    // error.
    void printErrorMessage(const std::string& location);
}
