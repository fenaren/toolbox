...
private:
    ...
    // of memory is read each time).  Because we should have written this memory
    // in the byte ordering that is opposite of this host system, this T has to
    // be byteswapped again to be sensical.  This value is then left alone to be
    // returned in the "dptest_var" argument.
    //
    // Second, we save a copy of an incremented "dptest_var" into
    // "dptest_var_updated".  This value is then written byteswapped into the
    // same memory location we read as described in the paragraph above, meaning
    // the memory location is now incremented but otherwise unchanged.  Later,
    // readRaw() will read this, and we can test for presence of the incremented
    // value.
    //
